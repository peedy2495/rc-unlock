...
# Copy Python
COPY --from=builder /opt/python/bin/python3.12 /usr/local/bin/python3.12
COPY --from=builder /opt/python/lib/python3.12 /usr/local/lib/python3.12
...
HEALTHCHECK --interval=30s --timeout=3s --start-period=5s --retries=3 \
  CMD ["/usr/local/bin/python3.12", "-c", "import yaml, paramiko; import sys; sys.exit(0)"]

# Set entrypoint using system Python with venv packages
ENTRYPOINT ["/usr/local/bin/python3.12", "rc-unlock.py"]